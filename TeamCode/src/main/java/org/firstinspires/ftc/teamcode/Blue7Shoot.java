...
@Autonomous(name = "Blue7Shoot", group = "Auto")

public class Blue7Shoot extends LinearOpMode {
    ...
    */
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && robot.MotorL.getCurrentPosition() > startPosL - 4350 && !doneDrive1) {//stop if hit line or go certain distance
            ...
            telemetry.addData("sensorColor:", robot.ColSensor.green());
            telemetry.update();
        ...
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() > startPosL - 640) && doneDrive1) {//only does this step if hit white line
            telemetry.addData("Status:", status);
            telemetry.addData("MotorL units to go", robot.MotorL.getCurrentPosition() - startPosL + 640);
        ...
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() < startPosL + 1700) && !turnInto1Hit) {
            if(robot.ColSensor.green() > 13){
                turnInto1Hit= true;//true if hit white line
            }
            telemetry.addData("change in time", runtime.seconds() - lastClock);
            telemetry.addData("Status:", status);
            telemetry.update();
            robot.MotorL.setPower(.45 * vl);
            robot.MotorR.setPower(-.45 * vr);
        ...
        status = "turn back if missed the white line the fist time";
        telemetry.update();
        startPosL = robot.MotorL.getCurrentPosition();
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() > startPosL - 850) && !turnInto1Hit) {
            robot.MotorL.setPower(-.5 * vl);
            robot.MotorR.setPower(.5 * vr);
            telemetry.addData("Status:", status);
            telemetry.addData("MotorL to go", robot.MotorL.getCurrentPosition() + startPosL + 290);
            telemetry.update();
        ...
        status = "drive backward if missed the white line the fist time";
        telemetry.update();
        startPosL = robot.MotorL.getCurrentPosition();
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() > startPosL - 600) && !turnInto1Hit) {
            robot.MotorL.setPower(-.5 * vl);
            robot.MotorR.setPower(-.5 * vr);
            telemetry.addData("Status:", status);
            telemetry.addData("MotorL to go", robot.MotorL.getCurrentPosition() + startPosL + 400);
            telemetry.update();
        ...
        status = "turn until white line, second try";
        robot.TouchServo.setPosition(1);//out
        turnOneCount = 0;
        lastPosL = robot.MotorL.getCurrentPosition();
        lastPosR = robot.MotorR.getCurrentPosition();
        startPosL = robot.MotorL.getCurrentPosition();
        lastClock = runtime.seconds();
        while (opModeIsActive() && (robot.ColSensor.green() < 13) && !turnInto1Hit) {
            telemetry.addData("change in time", runtime.seconds() - lastClock);
            telemetry.addData("Status:", status);
            telemetry.update();
            robot.MotorL.setPower(.45 * vl);
        ...
        while (opModeIsActive() && !robot.TouSensor.isPressed()) {
            ...
            telemetry.update();
            if (robot.ColSensor.green() < 13) {//grey
                robot.MotorR.setPower(-.4 * vl);
                robot.MotorL.setPower(.2 * vr);
            } else if (robot.ColSensor.green() > 13) {//white
        ...
        while (opModeIsActive() && runtime.seconds() < .1) {
            ...
            telemetry.addData("sensorColor:", robot.ColSensor.green());
            telemetry.update();
            if (robot.FruitySensor.blue() < robot.FruitySensor.red()) {//uses name beaconOne"Red" but inequality switched so blue
                beaconOneRed = false;
            } else {
                beaconOneRed = true;
            }
        ...
        lastClock = runtime.seconds();
        robot.TouchServo.setPosition(.1);//in
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() < startPosL + 300)) {
        ...
        lastPosR = robot.MotorR.getCurrentPosition();
        lastClock = runtime.seconds();
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() > startPosL - 500) && !beaconOneDone) {
            robot.MotorL.setPower(-1 * vl);
            robot.MotorR.setPower(-1 * vr);

            telemetry.addData("Status:", status);
            telemetry.addData("currentPos - startPosL + 400", robot.MotorL.getCurrentPosition() - startPosL + 260);
        ...
        startPosL = robot.MotorL.getCurrentPosition();
        runtime.reset();
        while (opModeIsActive() && (runtime.seconds() < 5) && beaconOneWrong ) {
            robot.MotorL.setPower(0);
            robot.MotorR.setPower(0);
            telemetry.addData("Status:", status);
            telemetry.update();
        ...
        lastPosR = robot.MotorR.getCurrentPosition();
        lastClock = runtime.seconds();
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() > startPosL - 220) && beaconOneWrong) {
            robot.MotorL.setPower(-1 * vl);
            robot.MotorR.setPower(-1 * vr);
        ...
        while (opModeIsActive() && robot.MotorL.getCurrentPosition() > startPosL - 700) {
            ...
            robot.PressServoL.setPosition(0);//in
            telemetry.addData("Status:", status);
            telemetry.addData("MotorL to go", robot.MotorL.getCurrentPosition() - startPosL + 700);
            telemetry.update();
        ...
        startPosL = robot.MotorL.getCurrentPosition();
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() > startPosL - 3650) && !longDriveDone) {//stop if hit line or go certain distance
            robot.MotorL.setPower(-1 * vl);
            robot.MotorR.setPower(-1 * vr);
            if (robot.ColSensor.green() > 13) {
        ...
        robot.MotorL.setPower(-.3 * vl);
        robot.MotorR.setPower(-.3 * vr);
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() > startPosL - 650) && longDriveDone) {//stop if go certain distance or stuck on wall
            telemetry.addData("Status:", status);
        ...
        lastPosR = robot.MotorR.getCurrentPosition();
        lastClock = runtime.seconds();
        while (opModeIsActive() && robot.ColSensor.green() < 13) {
            telemetry.addData("change in time", runtime.seconds() - lastClock);
            telemetry.addData("Status:", status);
            telemetry.update();
        ...
        lastPosR = robot.MotorR.getCurrentPosition();
        lastClock = runtime.seconds();
        while (opModeIsActive() && (robot.MotorL.getCurrentPosition() > startPosL - 500) && !beaconTwoDone) {
            robot.MotorL.setPower(-1 * vl);
            robot.MotorR.setPower(-1 * vr);
            telemetry.addData("Status:", status);
            telemetry.addData("currentPos - startPosL + 400", robot.MotorL.getCurrentPosition() - startPosL + 200);
        ...
        startPosL = robot.MotorL.getCurrentPosition();
        runtime.reset();
        while (opModeIsActive() && (runtime.seconds() < 5) && beaconTwoWrong ) {
            robot.MotorL.setPower(0);
            robot.MotorR.setPower(0);
            telemetry.addData("Status:", status);
            telemetry.addData("MotorL to go", robot.MotorL.getCurrentPosition() - startPosL + 1250);
